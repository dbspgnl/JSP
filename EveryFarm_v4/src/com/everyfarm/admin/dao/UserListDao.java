...
private String namespace = "admin.";
	...
	public List<UserListDto> selectList(int from, int to, String userName){
		List<UserListDto> list = null;
		SqlSession session = null;
//		PagingDto pagingdto = new PagingDto();
//		pagingdto.setFrom(from);
...
//		pagingdto.setUserName(userName);
		...
		try {
			...
			list = session.selectList(namespace+"userlist_search",hashmap);
		} catch (Exception e) {
			System.out.println("ERROR: UserListDao selectList2");
			e.printStackTrace();
		} finally {
			session.close();
		}
