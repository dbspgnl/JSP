			 ...
			 try {
		            ...
		            msg.setText(new StringBuffer().append("<fieldset style='text-align:center; height: 1100px;'>")
						...
						.append("<button style=\"color:white; background:black; font-weight:bold; width:150px; height:40px; font-size:18px; border-radius:10px;\">"
								+ "<a style=\"color:white; text-decoration: none;\" href=\"http://localhost:8787/EveryFarm_v4/signup.do?command=keysend\">Click</a></button>")
						.append("</fieldset>").toString()
		            		);
		            msg.setHeader("Content-type", "text/html; charset=UTF-8");
		            Transport.send(msg);
		            System.out.println("<이메일 전송 성공>");
		            response.sendRedirect("signup/signupform.jsp?emailNotTobe="+emailNotTobe); // 이메일 중복 response
		        } catch (Exception e) {
		        	e.printStackTrace();
